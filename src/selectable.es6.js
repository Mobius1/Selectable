...
export default class Selectable extends Emitter {
	...
	refresh(skip = false) {
		...
		this.scroll = {
			x, y,
			max: { x: this.scrollWidth - this.clientWidth, y: this.scrollHeight - this.clientHeight },
			size: { x: this.clientWidth, y: this.clientHeight },
			scrollable: { x: this.scrollWidth > this.offsetWidth, y: this.scrollHeight > this.offsetHeight }
		};
	...
	drag(e) {
		...
		this.mouse = { x: evt.pageX, y: evt.pageY };
